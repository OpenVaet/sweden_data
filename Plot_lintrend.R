...
# Calculate the linear trend for 2010-2020 for each age_group
for (age_group in names(age_groups)) {
  subset_data <- subset(age_groups[[age_group]], year >= 2010 & year <= 2020)
...
actual_colors <- c("15-24, Actual" = "blue", "25-34, Actual" = "green", "35+, Actual" = "red")
trend_colors <- c("15-24, Trend" = "lightblue", "25-34, Trend" = "lightgreen", "35+, Trend" = "pink")
...
p <- ggplot(combined_data, aes(x = year, y = births, group = label)) +
  ...
  labs(title = "Sweden - Births by Age Group 2021 & 2022 with Linear Trend (2010-2020)", x = "Year", y = "Number of Births", color = "Legend") +
  theme_minimal() +
