...
trend_data <- do.call(rbind, lapply(names(trends), function(age_group) {
  data.frame(age_group = age_group, year = 2010:2023, births_per_1000 = predict(trends[[age_group]], newdata = data.frame(year = 2010:2023)), type = "Trend")
}))

# Calculate the percentage of deviation to the linear trend for 2020, 2021, 2022 & 2023
...
p <- ggplot(combined_data, aes(x = year, y = births_per_1000, group = label)) +
  ...
  labs(title = "Sweden - Births / 1000 Women in Age, by Age Group 2020, 2021, 2022 & 2023 with Linear Trend (2010-2019)", x = "Year", y = "Number of Births", color = "Legend") +
  theme_minimal() +
